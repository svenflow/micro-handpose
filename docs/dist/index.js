function be(s){return s.replace(/\/\/[^\n]*/g,"").replace(/\s+/g," ").replace(/\s*([{}();,=+\-*/<>!&|@])\s*/g,"$1").trim()}function Ta(s){let _=["weight","bias","pw_weight","pw_bias","dw_weight","dw_bias","handflag_w","handflag_b","handedness_w","handedness_b","landmarks_w","landmarks_b"],a="enable f16;"+s;for(let B of _)for(;a.includes(`${B}:array<f32>`);)a=a.replace(`${B}:array<f32>`,`${B}:array<f16>`);return a}var _a=be(`
...
`);function La(s,_){return fa.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${s},${_},1)`)}function Ra(s,_){return _a.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${s},${_},1)`)}function Oa(s,_){return ma.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${s},${_},1)`)}function Ia(s,_){return ha.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${s},${_},1)`)}function Fa(s,_){return[8,8]}var za=be(`
...
`)}var Za=Va("sigmoid"),ja=Va("div256"),Ja=be(`
@group(0)@binding(0) var<storage,read> input:array<f32>;
...
`);function en(s,_){let B=Math.min(_,256),x=_>B,v=s%4===0?`var ic:u32=0u;
  ...
  ${L}`,C=x?"}":l,D=x?`for(var c:u32=lid.x;c<${_}u;c+=${B}u){`:"{let c=lid.x;";return be(`
...
`);async function wa(s,_){let a;if(_)a=_;else{if(!navigator.gpu)throw new Error("WebGPU not supported");let d=await navigator.gpu.requestAdapter();if(!d)throw new Error("No GPU adapter found");a=await d.requestDevice({requiredLimits:{maxStorageBuffersPerShaderStage:Math.min(d.limits.maxStorageBuffersPerShaderStage,8)}})}let B={r:"read-only-storage",s:"storage",u:"uniform"};function x(d){return a.createBindGroupLayout({entries:d.map((n,g)=>n==="t"?{binding:g,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float"}}:{binding:g,visibility:GPUShaderStage.COMPUTE,buffer:{type:B[n]}})})}let e=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC,v=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC,T=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC,L=GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST;function l(d,n){return a.createBuffer({size:Math.max(d,4),usage:n})}function b(d,n,g){a.queue.writeBuffer(d,n,g)}function C(d){let n=l(d.data.byteLength,e);return b(n,0,d.data),n}let D=Array.from(s.keys());function K(d){let n=s.get(d);if(!n)throw new Error(`Weight not found: ${d}`);return n}function m(...d){let n=D.find(g=>d.every(S=>g.includes(S)));if(!n)throw new Error(`Weight not found for: ${d.join(", ")}`);return K(n)}function Y(d){let[,n,g,S]=d.shape,h=new Float32Array(S*25);for(let r=0;r<S;r++)for(let I=0;I<n;I++)for(let ie=0;ie<g;ie++)h[r*25+I*5+ie]=d.data[I*g*S+ie*S+r];return h}function G(d){let[n,,,g]=d.shape,S=new Float32Array(n*g);for(let h=0;h<n;h++)for(let r=0;r<g;r++)S[h*g+r]=d.data[h*g+r];return S}let ce=a.createShaderModule({code:sn}),re=a.createShaderModule({code:on}),ye=a.createShaderModule({code:un}),oe=a.createShaderModule({code:pn}),W=a.createShaderModule({code:dn}),te=a.createShaderModule({code:cn}),ue=a.createShaderModule({code:ln}),O=x(["r","r","r","r","s","u"]),X=x(["r","r","r","s","u"]),j=x(["r","r","r","r","r","s","u"]),Te=x(["r","r","r","s","u"]),ve=x(["r","r","r","r","s","u"]),me=x(["r","r","s","u"]),xe=x(["t","s","u"]);function Ue(d,n){return a.createComputePipeline({layout:a.createPipelineLayout({bindGroupLayouts:[d]}),compute:{module:n,entryPoint:"main"}})}let k=Ue(O,ce),V=Ue(X,re),he=Ue(j,ye),J=Ue(Te,oe),de=Ue(ve,W),Q=Ue(me,te),Oe=Ue(xe,ue),tt=m("conv2d/Conv2D"),Ge=m("batch_normalization/","conv2d/Conv2D"),Ae=m("p_re_lu/"),Pe=C(tt),Be=C(Ge),Le=C(Ae),Se=[{dwKey:"depthwise_conv2d/",pwKey:"conv2d_1/",bnKey:"batch_normalization_1/",preluKey:"p_re_lu_1/",inCh:32,outCh:32,stride:1,inH:96},{dwKey:"depthwise_conv2d_1/",pwKey:"conv2d_2/",bnKey:"batch_normalization_2/",preluKey:"p_re_lu_2/",inCh:32,outCh:32,stride:1,inH:96},{dwKey:"depthwise_conv2d_2/",pwKey:"conv2d_3/",bnKey:"batch_normalization_3/",preluKey:"p_re_lu_3/",inCh:32,outCh:32,stride:1,inH:96},{dwKey:"depthwise_conv2d_3/",pwKey:"conv2d_4/",bnKey:"batch_normalization_4/",preluKey:"p_re_lu_4/",inCh:32,outCh:64,stride:2,inH:96},{dwKey:"depthwise_conv2d_4/",pwKey:"conv2d_5/",bnKey:"batch_normalization_5/",preluKey:"p_re_lu_5/",inCh:64,outCh:64,stride:1,inH:48},{dwKey:"depthwise_conv2d_5/",pwKey:"conv2d_6/",bnKey:"batch_normalization_6/",preluKey:"p_re_lu_6/",inCh:64,outCh:64,stride:1,inH:48},{dwKey:"depthwise_conv2d_6/",pwKey:"conv2d_7/",bnKey:"batch_normalization_7/",preluKey:"p_re_lu_7/",inCh:64,outCh:64,stride:1,inH:48},{dwKey:"depthwise_conv2d_7/",pwKey:"conv2d_8/",bnKey:"batch_normalization_8/",preluKey:"p_re_lu_8/",inCh:64,outCh:128,stride:2,inH:48},{dwKey:"depthwise_conv2d_8/",pwKey:"conv2d_9/",bnKey:"batch_normalization_9/",preluKey:"p_re_lu_9/",inCh:128,outCh:128,stride:1,inH:24},{dwKey:"depthwise_conv2d_9/",pwKey:"conv2d_10/",bnKey:"batch_normalization_10/",preluKey:"p_re_lu_10/",inCh:128,outCh:128,stride:1,inH:24},{dwKey:"depthwise_conv2d_10/",pwKey:"conv2d_11/",bnKey:"batch_normalization_11/",preluKey:"p_re_lu_11/",inCh:128,outCh:128,stride:1,inH:24},{dwKey:"depthwise_conv2d_11/",pwKey:"conv2d_12/",bnKey:"batch_normalization_12/",preluKey:"p_re_lu_12/",inCh:128,outCh:256,stride:2,inH:24},{dwKey:"depthwise_conv2d_12/",pwKey:"conv2d_13/",bnKey:"batch_normalization_13/",preluKey:"p_re_lu_13/",inCh:256,outCh:256,stride:1,inH:12},{dwKey:"depthwise_conv2d_13/",pwKey:"conv2d_14/",bnKey:"batch_normalization_14/",preluKey:"p_re_lu_14/",inCh:256,outCh:256,stride:1,inH:12},{dwKey:"depthwise_conv2d_14/",pwKey:"conv2d_15/",bnKey:"batch_normalization_15/",preluKey:"p_re_lu_15/",inCh:256,outCh:256,stride:1,inH:12},{dwKey:"depthwise_conv2d_15/",pwKey:"conv2d_16/",bnKey:"batch_normalization_16/",preluKey:"p_re_lu_16/",inCh:256,outCh:256,stride:2,inH:12},{dwKey:"depthwise_conv2d_16/",pwKey:"conv2d_17/",bnKey:"batch_normalization_17/",preluKey:"p_re_lu_17/",inCh:256,outCh:256,stride:1,inH:6},{dwKey:"depthwise_conv2d_17/",pwKey:"conv2d_18/",bnKey:"batch_normalization_18/",preluKey:"p_re_lu_18/",inCh:256,outCh:256,stride:1,inH:6},{dwKey:"depthwise_conv2d_18/",pwKey:"conv2d_19/",bnKey:"batch_normalization_19/",preluKey:"p_re_lu_19/",inCh:256,outCh:256,stride:1,inH:6}].map(d=>{let n=m(d.dwKey),g=m(d.pwKey),S=m(d.bnKey),h=m(d.preluKey),r=Y(n),I=l(r.byteLength,e);b(I,0,r);let ie=new Float32Array(d.inCh),f=l(ie.byteLength,e);b(f,0,ie);let A=G(g),y=l(A.byteLength,e);b(y,0,A);let N=C(S),E=C(h);return{dwWeightBuf:I,dwBiasBuf:f,pwWeightBuf:y,pwBiasBuf:N,alphaBuf:E,inCh:d.inCh,outCh:d.outCh,stride:d.stride,inH:d.inH}}),Ce=G(m("conv2d_20/Conv2D")),we=l(Ce.byteLength,e);b(we,0,Ce);let De=C(m("batch_normalization_20/")),Ve=C(m("p_re_lu_20/")),at={dwWeightBuf:(()=>{let d=Y(m("depthwise_conv2d_19/")),n=l(d.byteLength,e);return b(n,0,d),n})(),dwBiasBuf:(()=>{let d=new Float32Array(256),n=l(d.byteLength,e);return b(n,0,d),n})(),pwWeightBuf:(()=>{let d=G(m("conv2d_21/")),n=l(d.byteLength,e);return b(n,0,d),n})(),pwBiasBuf:C(m("batch_normalization_21/")),alphaBuf:C(m("p_re_lu_21/")),inCh:256,outCh:256,stride:1,inH:12},Ze={dwWeightBuf:(()=>{let d=Y(m("depthwise_conv2d_20/")),n=l(d.byteLength,e);return b(n,0,d),n})(),dwBiasBuf:(()=>{let d=new Float32Array(256),n=l(d.byteLength,e);return b(n,0,d),n})(),pwWeightBuf:(()=>{let d=G(m("conv2d_22/Conv2D1")),n=l(d.byteLength,e);return b(n,0,d),n})(),pwBiasBuf:C(m("batch_normalization_22/")),alphaBuf:C(m("p_re_lu_22/")),inCh:256,outCh:256,stride:1,inH:12},nt=G(m("conv2d_23/Conv2D")),ht=l(nt.byteLength,e);b(ht,0,nt);let it=C(m("batch_normalization_23/")),rt=C(m("p_re_lu_23/")),Ft={dwWeightBuf:(()=>{let d=Y(m("depthwise_conv2d_21/")),n=l(d.byteLength,e);return b(n,0,d),n})(),dwBiasBuf:(()=>{let d=new Float32Array(128),n=l(d.byteLength,e);return b(n,0,d),n})(),pwWeightBuf:(()=>{let d=G(m("conv2d_24/")),n=l(d.byteLength,e);return b(n,0,d),n})(),pwBiasBuf:C(m("batch_normalization_24/")),alphaBuf:C(m("p_re_lu_24/")),inCh:128,outCh:128,stride:1,inH:24},zt={dwWeightBuf:(()=>{let d=Y(m("depthwise_conv2d_22/")),n=l(d.byteLength,e);return b(n,0,d),n})(),dwBiasBuf:(()=>{let d=new Float32Array(128),n=l(d.byteLength,e);return b(n,0,d),n})(),pwWeightBuf:(()=>{let d=G(m("conv2d_25/Conv2D1")),n=l(d.byteLength,e);return b(n,0,d),n})(),pwBiasBuf:C(m("batch_normalization_25/")),alphaBuf:C(m("p_re_lu_25/")),inCh:128,outCh:128,stride:1,inH:24},Nt=G(m("classifier_palm_16_NO_PRUNING/Conv2D")),wt=l(Nt.byteLength,e);b(wt,0,Nt);let bt=C(m("classifier_palm_16_NO_PRUNING/BiasAdd")),Mt=G(m("regressor_palm_16_NO_PRUNING/Conv2D")),gt=l(Mt.byteLength,e);b(gt,0,Mt);let Et=C(m("regressor_palm_16_NO_PRUNING/BiasAdd")),st=G(m("classifier_palm_8_NO_PRUNING/Conv2D")),yt=l(st.byteLength,e);b(yt,0,st);let Wt=C(m("classifier_palm_8_NO_PRUNING/BiasAdd")),qt=G(m("regressor_palm_8_NO_PRUNING/Conv2D")),Ht=l(qt.byteLength,e);b(Ht,0,qt);let Me=C(m("regressor_palm_8_NO_PRUNING/BiasAdd")),xt=Math.max(36864*3,9216*64,2304*128,576*256,144*256)*4,je=l(36864*3*4,e),Ie=l(xt,v),Kt=l(xt,v),Ee=l(xt,v),vt=l(576*256*4,v),ot=l(144*256*4,v|GPUBufferUsage.COPY_DST),Fe=l(576*128*4,v|GPUBufferUsage.COPY_DST),ut=l(864*4,T),Re=l(15552*4,T),pt=l(576*2*4,T),ee=l(576*36*4,T),fe=l(864*4,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST),We=l(15552*4,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST),ct=l(576*2*4,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST),dt=l(576*36*4,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST),lt=a.createTexture({size:[192,192,1],format:"rgba8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT});function F(d,n){return Math.ceil(d/n)}function ne(d){let n=l(d.byteLength,L);return b(n,0,d),n}let ae=ne(new Uint32Array([1,3,32,192,192,96,96]));function pe(d,n,g,S,h){let r=n.stride===2?n.inH/2:n.inH,I=r,ie=n.stride===2?1:2,f=ne(new Uint32Array([1,n.inCh,n.inH,n.inH,r,I,n.stride,ie])),A=a.createBindGroup({layout:X,entries:[{binding:0,resource:{buffer:g}},{binding:1,resource:{buffer:n.dwWeightBuf}},{binding:2,resource:{buffer:n.dwBiasBuf}},{binding:3,resource:{buffer:Ee}},{binding:4,resource:{buffer:f}}]}),y=d.beginComputePass();y.setPipeline(V),y.setBindGroup(0,A),y.dispatchWorkgroups(F(I,8),F(r,8),n.inCh),y.end();let N=n.inCh,E=ne(new Uint32Array([1,n.inCh,n.outCh,r,I,N,n.stride,n.inH,n.inH])),ge=a.createBindGroup({layout:j,entries:[{binding:0,resource:{buffer:Ee}},{binding:1,resource:{buffer:h}},{binding:2,resource:{buffer:n.pwWeightBuf}},{binding:3,resource:{buffer:n.pwBiasBuf}},{binding:4,resource:{buffer:n.alphaBuf}},{binding:5,resource:{buffer:S}},{binding:6,resource:{buffer:E}}]}),le=d.beginComputePass();le.setPipeline(he),le.setBindGroup(0,ge),le.dispatchWorkgroups(F(I,8),F(r,8),n.outCh),le.end()}function He(d,n,g,S,h,r,I,ie,f){let A=ne(new Uint32Array([1,r,I,ie,f])),y=a.createBindGroup({layout:Te,entries:[{binding:0,resource:{buffer:n}},{binding:1,resource:{buffer:g}},{binding:2,resource:{buffer:S}},{binding:3,resource:{buffer:h}},{binding:4,resource:{buffer:A}}]}),N=d.beginComputePass();N.setPipeline(J),N.setBindGroup(0,y),N.dispatchWorkgroups(F(f,8),F(ie,8),I),N.end()}function _t(d,n,g,S,h,r,I,ie,f,A){let y=ne(new Uint32Array([1,I,ie,f,A])),N=a.createBindGroup({layout:ve,entries:[{binding:0,resource:{buffer:n}},{binding:1,resource:{buffer:g}},{binding:2,resource:{buffer:S}},{binding:3,resource:{buffer:h}},{binding:4,resource:{buffer:r}},{binding:5,resource:{buffer:y}}]}),E=d.beginComputePass();E.setPipeline(de),E.setBindGroup(0,N),E.dispatchWorkgroups(F(A,8),F(f,8),ie),E.end()}async function $t(d){a.queue.copyExternalImageToTexture({source:d},{texture:lt},[192,192]);let n=ne(new Uint32Array([192,192,192])),g=a.createBindGroup({layout:xe,entries:[{binding:0,resource:lt.createView()},{binding:1,resource:{buffer:je}},{binding:2,resource:{buffer:n}}]}),S=a.createCommandEncoder();{let P=S.beginComputePass();P.setPipeline(Oe),P.setBindGroup(0,g),P.dispatchWorkgroups(F(192,16),F(192,16),1),P.end()}{let P=a.createBindGroup({layout:O,entries:[{binding:0,resource:{buffer:je}},{binding:1,resource:{buffer:Pe}},{binding:2,resource:{buffer:Be}},{binding:3,resource:{buffer:Le}},{binding:4,resource:{buffer:Ie}},{binding:5,resource:{buffer:ae}}]}),Z=S.beginComputePass();Z.setPipeline(k),Z.setBindGroup(0,P),Z.dispatchWorkgroups(F(96,8),F(96,8),32),Z.end()}let h=Ie,r=Kt;for(let P=0;P<Se.length;P++){let Z=Se[P];pe(S,Z,h,r,h);let z=h;h=r,r=z,P===10&&S.copyBufferToBuffer(h,0,Fe,0,576*128*4),P===14&&S.copyBufferToBuffer(h,0,ot,0,144*256*4)}{let P=ne(new Uint32Array([1,256,6,6,12,12])),Z=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:vt}},{binding:2,resource:{buffer:r}},{binding:3,resource:{buffer:P}}]}),z=S.beginComputePass();z.setPipeline(Q),z.setBindGroup(0,Z),z.dispatchWorkgroups(F(12,8),F(12,8),256),z.end()}{let P=h;h=r,r=P}_t(S,h,we,De,Ve,r,256,256,12,12);{let P=h;h=r,r=P}{let P=ne(new Uint32Array([1,256,12,12,12,12])),Z=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:ot}},{binding:2,resource:{buffer:r}},{binding:3,resource:{buffer:P}}]}),z=S.beginComputePass();z.setPipeline(Q),z.setBindGroup(0,Z),z.dispatchWorkgroups(F(12,8),F(12,8),256),z.end()}{let P=h;h=r,r=P}pe(S,at,h,r,h);{let P=h;h=r,r=P}pe(S,Ze,h,r,h);{let P=h;h=r,r=P}He(S,h,wt,bt,ut,256,6,12,12),He(S,h,gt,Et,Re,256,108,12,12);{let P=ne(new Uint32Array([1,256,12,12,24,24])),Z=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:vt}},{binding:2,resource:{buffer:r}},{binding:3,resource:{buffer:P}}]}),z=S.beginComputePass();z.setPipeline(Q),z.setBindGroup(0,Z),z.dispatchWorkgroups(F(24,8),F(24,8),256),z.end()}{let P=h;h=r,r=P}_t(S,h,ht,it,rt,r,256,128,24,24);{let P=h;h=r,r=P}{let P=ne(new Uint32Array([1,128,24,24,24,24])),Z=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:Fe}},{binding:2,resource:{buffer:r}},{binding:3,resource:{buffer:P}}]}),z=S.beginComputePass();z.setPipeline(Q),z.setBindGroup(0,Z),z.dispatchWorkgroups(F(24,8),F(24,8),128),z.end()}{let P=h;h=r,r=P}pe(S,Ft,h,r,h);{let P=h;h=r,r=P}pe(S,zt,h,r,h);{let P=h;h=r,r=P}He(S,h,yt,Wt,pt,128,2,24,24),He(S,h,Ht,Me,ee,128,36,24,24),a.queue.submit([S.finish()]);let I=a.createCommandEncoder();I.copyBufferToBuffer(ut,0,fe,0,864*4),I.copyBufferToBuffer(Re,0,We,0,15552*4),I.copyBufferToBuffer(pt,0,ct,0,576*2*4),I.copyBufferToBuffer(ee,0,dt,0,576*36*4),a.queue.submit([I.finish()]),await Promise.all([fe.mapAsync(GPUMapMode.READ),We.mapAsync(GPUMapMode.READ),ct.mapAsync(GPUMapMode.READ),dt.mapAsync(GPUMapMode.READ)]);let ie=new Float32Array(fe.getMappedRange()).slice(),f=new Float32Array(We.getMappedRange()).slice(),A=new Float32Array(ct.getMappedRange()).slice(),y=new Float32Array(dt.getMappedRange()).slice();fe.unmap(),We.unmap(),ct.unmap(),dt.unmap();let N=2016,E=new Float32Array(N),ge=new Float32Array(N*18),le=0;for(let P=0;P<12;P++)for(let Z=0;Z<12;Z++)for(let z=0;z<6;z++){E[le]=ie[z*144+P*12+Z];for(let ke=0;ke<18;ke++){let Pt=z*18+ke;ge[le*18+ke]=f[Pt*144+P*12+Z]}le++}for(let P=0;P<24;P++)for(let Z=0;Z<24;Z++)for(let z=0;z<2;z++){E[le]=A[z*576+P*24+Z];for(let ke=0;ke<18;ke++){let Pt=z*18+ke;ge[le*18+ke]=y[Pt*576+P*24+Z]}le++}return{scores:E,regressors:ge}}async function $(d,n){let g=a.createBuffer({size:n*4,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),S=a.createCommandEncoder();S.copyBufferToBuffer(d,0,g,0,n*4),a.queue.submit([S.finish()]),await g.mapAsync(GPUMapMode.READ);let h=new Float32Array(g.getMappedRange()).slice();return g.unmap(),g.destroy(),h}async function Yt(d){a.queue.copyExternalImageToTexture({source:d},{texture:lt},[192,192]);function n(y,N=1e3){let E=y.slice(0,N);return{min:Math.min(...E),max:Math.max(...E),mean:E.reduce((ge,le)=>ge+le,0)/E.length,nonZero:E.filter(ge=>ge!==0).length,sample:Array.from(E.slice(0,10))}}let g={},S=ne(new Uint32Array([192,192,192])),h=a.createBindGroup({layout:xe,entries:[{binding:0,resource:lt.createView()},{binding:1,resource:{buffer:je}},{binding:2,resource:{buffer:S}}]}),r=a.createCommandEncoder(),I=r.beginComputePass();I.setPipeline(Oe),I.setBindGroup(0,h),I.dispatchWorkgroups(F(192,16),F(192,16),1),I.end(),a.queue.submit([r.finish()]),g.input=n(await $(je,36864*3)),r=a.createCommandEncoder();let ie=a.createBindGroup({layout:O,entries:[{binding:0,resource:{buffer:je}},{binding:1,resource:{buffer:Pe}},{binding:2,resource:{buffer:Be}},{binding:3,resource:{buffer:Le}},{binding:4,resource:{buffer:Ie}},{binding:5,resource:{buffer:ae}}]});I=r.beginComputePass(),I.setPipeline(k),I.setBindGroup(0,ie),I.dispatchWorkgroups(F(96,8),F(96,8),32),I.end(),a.queue.submit([r.finish()]),g.initConv=n(await $(Ie,9216*32));let f=Ie,A=Kt;for(let y=0;y<Se.length;y++){let N=Se[y];r=a.createCommandEncoder(),pe(r,N,f,A,f),a.queue.submit([r.finish()]);let E=f;if(f=A,A=E,y===0||y===3||y===7||y===11||y===14||y===15||y===18){let ge=N.stride===2?N.inH/2:N.inH,le=ge*ge*N.outCh;g[`block${y}`]=n(await $(f,le))}y===10&&(r=a.createCommandEncoder(),r.copyBufferToBuffer(f,0,Fe,0,576*128*4),a.queue.submit([r.finish()])),y===14&&(r=a.createCommandEncoder(),r.copyBufferToBuffer(f,0,ot,0,144*256*4),a.queue.submit([r.finish()]))}r=a.createCommandEncoder();{let y=ne(new Uint32Array([1,256,6,6,12,12])),N=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:f}},{binding:1,resource:{buffer:vt}},{binding:2,resource:{buffer:A}},{binding:3,resource:{buffer:y}}]}),E=r.beginComputePass();E.setPipeline(Q),E.setBindGroup(0,N),E.dispatchWorkgroups(F(12,8),F(12,8),256),E.end()}a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpnUpsample6to12=n(await $(f,144*256)),r=a.createCommandEncoder(),_t(r,f,we,De,Ve,A,256,256,12,12),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpn6to12Conv=n(await $(f,144*256)),g.backbone12Skip=n(await $(ot,144*256)),r=a.createCommandEncoder();{let y=ne(new Uint32Array([1,256,12,12,12,12])),N=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:f}},{binding:1,resource:{buffer:ot}},{binding:2,resource:{buffer:A}},{binding:3,resource:{buffer:y}}]}),E=r.beginComputePass();E.setPipeline(Q),E.setBindGroup(0,N),E.dispatchWorkgroups(F(12,8),F(12,8),256),E.end()}a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpnAdd12=n(await $(f,144*256)),r=a.createCommandEncoder(),pe(r,at,f,A,f),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpn12Block1=n(await $(f,144*256)),r=a.createCommandEncoder(),pe(r,Ze,f,A,f),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpn12Block2=n(await $(f,144*256)),r=a.createCommandEncoder(),He(r,f,wt,bt,ut,256,6,12,12),a.queue.submit([r.finish()]),g.cls16=n(await $(ut,864)),r=a.createCommandEncoder(),He(r,f,gt,Et,Re,256,108,12,12),a.queue.submit([r.finish()]),g.reg16=n(await $(Re,15552),500),r=a.createCommandEncoder();{let y=ne(new Uint32Array([1,256,12,12,24,24])),N=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:f}},{binding:1,resource:{buffer:vt}},{binding:2,resource:{buffer:A}},{binding:3,resource:{buffer:y}}]}),E=r.beginComputePass();E.setPipeline(Q),E.setBindGroup(0,N),E.dispatchWorkgroups(F(24,8),F(24,8),256),E.end()}a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpnUpsample12to24=n(await $(f,576*256)),r=a.createCommandEncoder(),_t(r,f,ht,it,rt,A,256,128,24,24),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpn12to24Conv=n(await $(f,576*128)),g.backbone24Skip=n(await $(Fe,576*128)),r=a.createCommandEncoder();{let y=ne(new Uint32Array([1,128,24,24,24,24])),N=a.createBindGroup({layout:me,entries:[{binding:0,resource:{buffer:f}},{binding:1,resource:{buffer:Fe}},{binding:2,resource:{buffer:A}},{binding:3,resource:{buffer:y}}]}),E=r.beginComputePass();E.setPipeline(Q),E.setBindGroup(0,N),E.dispatchWorkgroups(F(24,8),F(24,8),128),E.end()}a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpnAdd24=n(await $(f,576*128)),r=a.createCommandEncoder(),pe(r,Ft,f,A,f),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}g.fpn24Block1=n(await $(f,576*128)),r=a.createCommandEncoder(),pe(r,zt,f,A,f),a.queue.submit([r.finish()]);{let y=f;f=A,A=y}return g.fpn24Block2=n(await $(f,576*128)),r=a.createCommandEncoder(),He(r,f,yt,Wt,pt,128,2,24,24),a.queue.submit([r.finish()]),g.cls8=n(await $(pt,576*2)),r=a.createCommandEncoder(),He(r,f,Ht,Me,ee,128,36,24,24),a.queue.submit([r.finish()]),g.reg8=n(await $(ee,576*36)),g.initWeights=n(await $(Pe,100),100),g.initBias=n(await $(Be,32),32),g.cls16Weights=n(await $(wt,100),100),g.cls16Bias=n(await $(bt,6),6),g.cls8Weights=n(await $(yt,100),100),g.cls8Bias=n(await $(Wt,2),2),g.fpn6to12Weights=n(await $(we,100),100),g}return{device:a,run:$t,debugRun:Yt}}function qn(){let s=[];for(let _=0;_<12;_++)for(let a=0;a<12;a++){let B=(a+.5)/12,x=(_+.5)/12;for(let e=0;e<6;e++)s.push({x:B,y:x})}for(let _=0;_<24;_++)for(let a=0;a<24;a++){let B=(a+.5)/24,x=(_+.5)/24;for(let e=0;e<2;e++)s.push({x:B,y:x})}return s}var _n=qn();function Kn(s){return 1/(1+Math.exp(-s))}function fn(s,_){let a=[],{scores:B,regressors:x}=s,e=192;for(let v=0;v<_n.length;v++){let T=Kn(B[v]);if(T<_)continue;let L=_n[v],l=v*18,b=L.x+x[l+0]/e,C=L.y+x[l+1]/e,D=x[l+2]/e,K=x[l+3]/e,m=[];for(let Y=0;Y<7;Y++){let G=L.x+x[l+4+Y*2]/e,ce=L.y+x[l+4+Y*2+1]/e;m.push([G,ce])}a.push({score:T,box:[b,C,D,K],keypoints:m})}return a}function mn(s,_){if(s.length===0)return[];let a=[...s].sort((e,v)=>v.score-e.score),B=[],x=new Set;for(let e=0;e<a.length;e++)if(!x.has(e)){B.push(a[e]);for(let v=e+1;v<a.length;v++)x.has(v)||$n(a[e],a[v])>_&&x.add(v)}return B}function $n(s,_){let a=s.box[0]-s.box[2]/2,B=s.box[1]-s.box[3]/2,x=s.box[0]+s.box[2]/2,e=s.box[1]+s.box[3]/2,v=_.box[0]-_.box[2]/2,T=_.box[1]-_.box[3]/2,L=_.box[0]+_.box[2]/2,l=_.box[1]+_.box[3]/2,b=Math.max(a,v),C=Math.max(B,T),D=Math.min(x,L),K=Math.min(e,l),m=Math.max(0,D-b),Y=Math.max(0,K-C),G=m*Y,ce=(x-a)*(e-B),re=(L-v)*(l-T),ye=ce+re-G;return ye>0?G/ye:0}function Yn(s){let[_,a,B,x]=s.box,e=s.keypoints[0],v=s.keypoints[2],T=v[0]-e[0],L=v[1]-e[1],l=Math.atan2(L,T),C=-Math.PI/2-l,D=Math.max(B,x),m=D*2.6,Y=-.5*D,G=Math.cos(C),ce=Math.sin(C),re=-Y*ce,ye=Y*G;return{centerX:_+re,centerY:a+ye,width:m,height:m,rotation:C}}function ba(s,_={}){let{scoreThreshold:a=.5,nmsThreshold:B=.3,maxHands:x=2}=_;async function e(T){let L=await s.run(T),l=fn(L,a);return mn(l,B).slice(0,x).map(Yn)}async function v(T){let L=await s.run(T),l=fn(L,a);return mn(l,B).slice(0,x)}return{detect:e,detectRaw:v,model:s}}function hn(s,_=256){let a=Math.cos(s.rotation),B=Math.sin(s.rotation),x=s.width/_,e=s.height/_,v=x*a,T=-e*B,L=x*B,l=e*a,b=s.centerX-(v*_/2+T*_/2),C=s.centerY-(L*_/2+l*_/2),D=v*l-T*L,K=l/D,m=-T/D,Y=-L/D,G=v/D,ce=-(K*b+m*C),re=-(Y*b+G*C);return{forward:[v,T,b,L,l,C],inverse:[K,m,ce,Y,G,re]}}function ga(s,_){let{forward:a}=hn(_,1),[B,x,e,v,T,L]=a;return s.map(l=>({x:B*l.x+x*l.y+e,y:v*l.x+T*l.y+L,z:l.z}))}var wn="https://cdn.jsdelivr.net/npm/@svenflow/micro-handpose@latest/weights";async function Xn(s={}){let{weightsUrl:_,scoreThreshold:a=.5,forceF32:B=!1}=s;if(typeof navigator>"u"||!navigator.gpu)throw new Error("micro-handpose requires WebGPU. Check browser support at https://webgpureport.org");let x=_??wn,e=x.endsWith("/")?x:`${x}/`,v=`${e}weights_f16.json`,T=`${e}weights_f16.bin`,[L,l]=await Promise.all([fetch(v),fetch(T)]);if(!L.ok)throw new Error(`Failed to fetch weights metadata: ${L.status}`);if(!l.ok)throw new Error(`Failed to fetch weights binary: ${l.status}`);let b=await L.json(),C=await l.arrayBuffer(),D=Ot(b,C),K=await It(D,{forceF32:B});if(!B){let O=new OffscreenCanvas(256,256),X=O.getContext("2d");X.fillStyle="#886644",X.fillRect(0,0,256,256),X.fillStyle="#cc9966",X.fillRect(50,50,156,156);let j=await K.runFromCanvas(O);j.landmarks.every(ve=>ve===0)&&j.handflag.every(ve=>ve===0)&&(console.warn("[micro-handpose] f16 model produced all-zero output \u2014 recompiling with f32"),K.device.destroy(),K=await It(D,{forceF32:!0}))}let m=null;function Y(){return m||(m=new OffscreenCanvas(256,256)),m}async function G(O){if(O instanceof HTMLCanvasElement||O instanceof OffscreenCanvas||typeof ImageBitmap<"u"&&O instanceof ImageBitmap)return O;let X=Y();X.width=256,X.height=256;let j=X.getContext("2d");return O instanceof ImageData?j.putImageData(O,0,0):j.drawImage(O,0,0,256,256),X}function ce(O,X,j){let Te=O[0];if(Te<a)return null;let ve=X[0]>.5,me=[];for(let xe=0;xe<21;xe++)me.push({x:j[xe*3],y:j[xe*3+1],z:j[xe*3+2]});return{score:Te,handedness:ve?"right":"left",landmarks:me}}async function re(O){let X=await G(O),j=await K.runFromCanvas(X);return ce(j.handflag,j.handedness,j.landmarks)}async function ye(O){let X=await G(O),j=await K.runFromCanvasPipelined(X);return j?ce(j.handflag,j.handedness,j.landmarks):null}async function oe(){let O=await K.flushPipelined();return O?ce(O.handflag,O.handedness,O.landmarks):null}function W(){K.device.destroy(),m=null}async function te(O){let X=await G(O);return K.benchmarkDiagnostic(X)}async function ue(O){let X=await G(O);return K.debugLayerOutputs(X)}return{detect:re,detectPipelined:ye,flushPipelined:oe,dispose:W,benchmarkDiagnostic:te,debugLayerOutputs:ue}}async function Vn(s={}){let{weightsUrl:_,palmWeightsUrl:a,scoreThreshold:B=.5,palmScoreThreshold:x=.5,maxHands:e=2,forceF32:v=!1}=s;if(typeof navigator>"u"||!navigator.gpu)throw new Error("micro-handpose requires WebGPU. Check browser support at https://webgpureport.org");let T=_??wn,L=T.endsWith("/")?T:`${T}/`;if(!a)throw new Error("palmWeightsUrl is required for createFullHandpose");let l=a.endsWith("/")?a:`${a}/`,[b,C,D,K]=await Promise.all([fetch(`${L}weights_f16.json`),fetch(`${L}weights_f16.bin`),fetch(`${l}palm_detection_weights.json`),fetch(`${l}palm_detection_weights.bin`)]);if(!b.ok)throw new Error(`Failed to fetch landmark weights metadata: ${b.status}`);if(!C.ok)throw new Error(`Failed to fetch landmark weights binary: ${C.status}`);if(!D.ok)throw new Error(`Failed to fetch palm weights metadata: ${D.status}`);if(!K.ok)throw new Error(`Failed to fetch palm weights binary: ${K.status}`);let[m,Y,G,ce]=await Promise.all([b.json(),C.arrayBuffer(),D.json(),K.arrayBuffer()]),re=Ot(m,Y),ye=Ot(G,ce),oe=await It(re,{forceF32:v});if(!v){let k=new OffscreenCanvas(256,256),V=k.getContext("2d");V.fillStyle="#886644",V.fillRect(0,0,256,256),V.fillStyle="#cc9966",V.fillRect(50,50,156,156);let he=await oe.runFromCanvas(k);he.landmarks.every(de=>de===0)&&he.handflag.every(de=>de===0)&&(console.warn("[micro-handpose] f16 model produced all-zero output \u2014 recompiling with f32"),oe.device.destroy(),oe=await It(re,{forceF32:!0}))}let W=await wa(ye),te=ba(W,{scoreThreshold:x,maxHands:e}),ue=null,O=null;function X(){return ue||(ue=new OffscreenCanvas(192,192)),ue}function j(){return O||(O=new OffscreenCanvas(256,256)),O}async function Te(k){if(k instanceof HTMLCanvasElement||k instanceof OffscreenCanvas){if(k.width===192&&k.height===192)return k;let J=X();return J.width=192,J.height=192,J.getContext("2d").drawImage(k,0,0,192,192),J}if(typeof ImageBitmap<"u"&&k instanceof ImageBitmap){if(k.width===192&&k.height===192)return k;let J=X();return J.width=192,J.height=192,J.getContext("2d").drawImage(k,0,0,192,192),J}let V=X();V.width=192,V.height=192;let he=V.getContext("2d");if(k instanceof ImageData){let J=new OffscreenCanvas(k.width,k.height);J.getContext("2d").putImageData(k,0,0),he.drawImage(J,0,0,192,192)}else he.drawImage(k,0,0,192,192);return V}function ve(k,V,he,J){let de=j();de.width=256,de.height=256;let Q=de.getContext("2d"),Oe=Math.cos(-V.rotation),tt=Math.sin(-V.rotation);Q.clearRect(0,0,256,256),Q.save(),Q.translate(128,128),Q.scale(V.width*he/256,V.height*J/256),Q.rotate(-V.rotation),Q.translate(-128,-128);let Ge=V.centerX*he,Ae=V.centerY*J;Q.restore();let Pe=Math.min(he,J),Be=256/(V.width*Pe),Le=256/(V.height*Pe),Xe=Math.cos(V.rotation),Se=Math.sin(V.rotation),Ce=Xe*Be,we=Se*Be,De=-Se*Le,Ve=Xe*Le,at=-Ge*Ce-Ae*De+128,Ze=-Ge*we-Ae*Ve+128;if(Q.setTransform(Ce,we,De,Ve,at,Ze),k instanceof ImageData){let nt=new OffscreenCanvas(k.width,k.height);nt.getContext("2d").putImageData(k,0,0),Q.drawImage(nt,0,0)}else Q.drawImage(k,0,0);return Q.setTransform(1,0,0,1,0,0),de}function me(k){return k instanceof HTMLCanvasElement||k instanceof OffscreenCanvas?[k.width,k.height]:typeof ImageBitmap<"u"&&k instanceof ImageBitmap?[k.width,k.height]:k instanceof ImageData?[k.width,k.height]:k instanceof HTMLVideoElement?[k.videoWidth,k.videoHeight]:k instanceof HTMLImageElement?[k.naturalWidth,k.naturalHeight]:[256,256]}async function xe(k){let V=await Te(k),he=await te.detect(V);if(he.length===0)return[];let[J,de]=me(k),Q=[];for(let Oe of he){let tt=ve(k,Oe,J,de),Ge=await oe.runFromCanvas(tt),Ae=Ge.handflag[0];if(Ae<B)continue;let Pe=Ge.handedness[0]>.5,Be=[];for(let we=0;we<21;we++)Be.push({x:Ge.landmarks[we*3],y:Ge.landmarks[we*3+1],z:Ge.landmarks[we*3+2]});let Le=Math.min(J,de),Xe=Oe.width*Le,Se={...Oe,width:Xe/J,height:Xe/de},Ce=ga(Be,Se);Q.push({score:Ae,handedness:Pe?"right":"left",landmarks:Ce,palmScore:0})}return Q}function Ue(){oe.device.destroy(),W.device.destroy(),ue=null,O=null}return{detect:xe,dispose:Ue}}function Zn(s){return s.replace(/\/\/[^\n]*/g,"").replace(/\s+/g," ").replace(/\s*([{}();,=+\-*/<>!&|@])\s*/g,"$1").trim()}var jn=Zn(`
