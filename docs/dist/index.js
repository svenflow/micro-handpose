function A(o){return o.replace(/\/\/[^\n]*/g,"").replace(/\s+/g," ").replace(/\s*([{}();,=+\-*/<>!&|@])\s*/g,"$1").trim()}function ut(o){let w=["weight","bias","pw_weight","pw_bias","dw_weight","dw_bias","handflag_w","handflag_b","handedness_w","handedness_b","landmarks_w","landmarks_b"],h="enable f16;"+o;for(let b of w)for(;h.includes(`${b}:array<f32>`);)h=h.replace(`${b}:array<f32>`,`${b}:array<f16>`);return h}var Qe=A(`
...
`);function ot(o,w){return ea.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${o},${w},1)`)}function pt(o,w){return Qe.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${o},${w},1)`)}function dt(o,w){return aa.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${o},${w},1)`)}function ct(o,w){return ta.replace("@compute @workgroup_size(8,8,1)",`@compute @workgroup_size(${o},${w},1)`)}function _t(o,w){return[8,8]}var lt=A(`
...
`)}var xt=yt("sigmoid"),vt=yt("div256"),Pt=A(`
@group(0)@binding(0) var<storage,read> input:array<f32>;
...
`);function Bt(o,w){let b=Math.min(w,256),U=w>b,W=o%4===0?`var ic:u32=0u;
    while(ic<${o}u){
      sum0+=shared_dw[ic]*pw_weight[pw_base+ic];
      sum1+=shared_dw[ic+1u]*pw_weight[pw_base+ic+1u];
      sum2+=shared_dw[ic+2u]*pw_weight[pw_base+ic+2u];
  ...
  ${E}`,X=U?"}":M,g=U?`for(var c:u32=lid.x;c<${w}u;c+=${b}u){`:"{let c=lid.x;";return A(`
...
}`,n=a.createShaderModule({code:t}),i=a.createShaderModule({code:s}),e=await n.getCompilationInfo(),c=await i.getCompilationInfo();if(e.messages.some(d=>d.type==="error")||c.messages.some(d=>d.type==="error"))W=!1;else{let d=new Float32Array(2400);d.fill(1);let _=new Uint16Array(2400);_.fill(10516);let p=new Uint16Array(96);p.fill(14336);let u=new Uint16Array(9216);u.fill(8478);let r=new Uint16Array(96);r.fill(12288);let m=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST,P=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC,C=a.createBuffer({size:d.byteLength,usage:m}),se=a.createBuffer({size:_.byteLength,usage:m}),ue=a.createBuffer({size:p.byteLength,usage:m}),oe=a.createBuffer({size:384,usage:GPUBufferUsage.STORAGE}),pe=a.createBuffer({size:u.byteLength,usage:m}),de=a.createBuffer({size:r.byteLength,usage:m}),ce=a.createBuffer({size:384,usage:P}),J=a.createBuffer({size:384,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});a.queue.writeBuffer(C,0,d),a.queue.writeBuffer(se,0,_),a.queue.writeBuffer(ue,0,p),a.queue.writeBuffer(pe,0,u),a.queue.writeBuffer(de,0,r);let Y="read-only-storage",he="storage",be=a.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:he}}]}),rt=a.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:Y}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:he}}]}),mi=a.createComputePipeline({layout:a.createPipelineLayout({bindGroupLayouts:[be]}),compute:{module:n,entryPoint:"main"}}),fi=a.createComputePipeline({layout:a.createPipelineLayout({bindGroupLayouts:[rt]}),compute:{module:i,entryPoint:"main"}}),hi=a.createBindGroup({layout:be,entries:[{binding:0,resource:{buffer:C}},{binding:1,resource:{buffer:se}},{binding:2,resource:{buffer:ue}},{binding:3,resource:{buffer:oe}}]}),bi=a.createBindGroup({layout:rt,entries:[{binding:0,resource:{buffer:oe}},{binding:1,resource:{buffer:pe}},{binding:2,resource:{buffer:de}},{binding:3,resource:{buffer:ce}}]}),We=a.createCommandEncoder(),Me=We.beginComputePass();Me.setPipeline(mi),Me.setBindGroup(0,hi),Me.dispatchWorkgroups(2),Me.end();let Ee=We.beginComputePass();Ee.setPipeline(fi),Ee.setBindGroup(0,bi),Ee.dispatchWorkgroups(2),Ee.end(),We.copyBufferToBuffer(ce,0,J,0,384),a.queue.submit([We.finish()]),await a.queue.onSubmittedWorkDone(),await J.mapAsync(GPUMapMode.READ);let ge=new Float32Array(J.getMappedRange()),st=1.5*.0104*96+.25,gi=ge[0]!==0&&ge[47]!==0&&ge[95]!==0,yi=Math.abs(ge[0]-st)<1;W=gi&&yi,J.unmap(),C.destroy(),se.destroy(),ue.destroy(),oe.destroy(),pe.destroy(),de.destroy(),ce.destroy(),J.destroy(),W||console.warn(`[micro-handpose] f16 multi-pass validation FAILED (got ${ge[0]}, expected ~${st.toFixed(2)}) \u2014 falling back to f32`)}}catch{W=!1}let E=o.values().next().value,M=W&&!!E?.rawF16&&!w?.forceF32;console.log(M?"[micro-handpose] Using f16 weight storage (shader-f16 validated)":`[micro-handpose] Using f32 weights (shader-f16 feature: ${b}, f16 validated: ${W}, f16 data: ${!!E?.rawF16})`);function x(t){if(M&&t.rawF16){let s=new Uint16Array(t.rawF16);if(s.length%2!==0){let n=new Uint16Array(s.length+1);return n.set(s),n}return s}return t.data}function X(t){if(M&&t.rawF16){let s=t.rawF16.byteLength;return Math.ceil(s/4)*4}return t.data.byteLength}function g(t){return M?ut(t):t}let H={r:"read-only-storage",s:"storage",u:"uniform"};function D(t){return a.createBindGroupLayout({entries:t.map((s,n)=>({binding:n,visibility:GPUShaderStage.COMPUTE,buffer:{type:H[s]}}))})}function He(t){return a.createBindGroupLayout({entries:t.map((s,n)=>s==="t"?{binding:n,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"float"}}:{binding:n,visibility:GPUShaderStage.COMPUTE,buffer:{type:H[s]}})})}let y=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST,_e=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC,le=GPUBufferUsage.STORAGE,ye=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC,L=GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST;function l(t,s){return a.createBuffer({size:t,usage:s})}function S(t,s){return a.createBindGroup({layout:t,entries:s.map((n,i)=>({binding:i,resource:"size"in n?{buffer:n}:n}))})}function T(t,s){return a.createComputePipeline({layout:a.createPipelineLayout({bindGroupLayouts:[t]}),compute:{module:s,entryPoint:"main"}})}let f=a.createShaderModule({code:lt}),v=a.createShaderModule({code:Gt}),k=a.createShaderModule({code:g(Pt)}),we=a.createShaderModule({code:g(ea)}),Q=a.createShaderModule({code:g(Qe)}),xe=a.createShaderModule({code:g(aa)}),ee=a.createShaderModule({code:g(ta)}),At=a.createShaderModule({code:g(wt)}),Wt=a.createShaderModule({code:ft}),Mt=a.createShaderModule({code:bt}),Et=a.createShaderModule({code:ht}),Ht=a.createShaderModule({code:g(gt)}),Tt=a.createShaderModule({code:g(xt)}),Lt=a.createShaderModule({code:g(vt)}),Ot=a.createShaderModule({code:g(kt)}),na=new Map;function Ci(t,s){let n=`${t}_${s}`,i=na.get(n);return i||(i=a.createShaderModule({code:g(Bt(t,s))}),na.set(n,i)),i}let Te=D(["r","r","r","s","u"]),Le=D(["r","r","r","r","s","u"]),ra=D(["r","s","u"]),sa=D(["r","r","r","s","u"]),Rt=D(["r","s","u"]),Ft=D(["r","r","s","u"]),ve=D(["r","r","s","u"]),ua=D(["r","r","r","s","u"]),me=D(["r","r","r","s","u"]),oa=He(["t","s","u"]),pa=D(["r","r","r","r","r","r","r","s"]),Oe=D(["r","r","r","r","r","s","u"]),It=a.createPipelineLayout({bindGroupLayouts:[Te]}),zt=a.createPipelineLayout({bindGroupLayouts:[Le]}),Pe=t=>a.createComputePipeline({layout:It,compute:{module:t,entryPoint:"main"}}),ke=t=>a.createComputePipeline({layout:zt,compute:{module:t,entryPoint:"main"}}),qt=Pe(we),$t=Pe(Q),Nt=ke(xe),Yt=ke(ee),da=new Map,ca=new Map,_a=new Map,la=new Map;da.set("8,8",qt),ca.set("8,8",$t),_a.set("8,8",Nt),la.set("8,8",Yt);function Be(t,s,n,i,e){let c=`${s},${n}`,d=t.get(c);return d||(d=e(a.createShaderModule({code:g(i(s,n))})),t.set(c,d)),d}let Xt=(t,s)=>Be(da,t,s,ot,Pe),Zt=(t,s)=>Be(ca,t,s,pt,Pe),Vt=(t,s)=>Be(_a,t,s,dt,ke),Kt=(t,s)=>Be(la,t,s,ct,ke),Z=Ct.map(t=>{let s=t.stride===2?t.h/2:t.h,n=t.stride===2?t.w/2:t.w,[i,e]=_t(t.inCh,s),c=t.h>=64,d=s>=16&&t.inCh>=288&&t.outCh>=288&&t.outCh%2===0;return{dwPipeline:c?Zt(i,e):Xt(i,e),pwPipeline:d?Kt(i,e):Vt(i,e),dwDispatchX:Math.ceil(n/i),dwDispatchY:Math.ceil(s/e),dwDispatchZ:t.inCh,pwDispatchX:Math.ceil(n/i),pwDispatchY:Math.ceil(s/e),pwDispatchZ:d?t.outCh/2:t.outCh}}),wa=T(ra,f),ma=T(sa,At);T(Rt,Wt),T(Ft,Mt);let Re=T(ve,Et),jt=T(ua,Ht);T(me,Tt),T(me,Lt);let V=T(oa,v),Jt=T(pa,k),Qt=T(Oe,Ot),Fe=1*288*128*128*4,fa=l(3*256*256*4,y),fe=l(3*257*257*4,le),ha=l(12,L);a.queue.writeBuffer(ha,0,new Uint32Array([3,256,257]));let G=l(Fe,_e),O=l(Fe,ye),te=l(Fe,le),ba=l(3072*64*4,y),ga=l(3072*32*4,y),ya=l(1536*16*4,y),xa=l(6144*64*4,le),ie=l(260,ye),B=l(260,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST);l(260,GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST);let F=a.createTexture({size:[256,256],format:"rgba8unorm",usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT}),va=l(8,L);a.queue.writeBuffer(va,0,new Uint32Array([256,257]));let Pa=o.get("backbone1.1.weight"),ka=o.get("backbone1.1.bias");if(!Pa||!ka)throw new Error("Missing input conv weights");let Ba=x(Pa),Ua=x(ka),Sa=l(Ba.byteLength,y),Ga=l(Ua.byteLength,y),Ca=l(28,L);a.queue.writeBuffer(Sa,0,Ba),a.queue.writeBuffer(Ga,0,Ua),a.queue.writeBuffer(Ca,0,new Uint32Array([1,3,24,257,257,128,128]));let Da=o.get("backbone6.1.weight"),Aa=o.get("backbone6.1.bias");if(!Da||!Aa)throw new Error("Missing backbone6.1 conv1x1 weights");let Wa=x(Da),Ma=x(Aa),Ea=l(Wa.byteLength,y),Ha=l(Ma.byteLength,y),Ta=l(20,L);a.queue.writeBuffer(Ea,0,Wa),a.queue.writeBuffer(Ha,0,Ma),a.queue.writeBuffer(Ta,0,new Uint32Array([1,96,48,32,32]));let La=o.get("handflag.weight"),Oa=o.get("handflag.bias");if(!La||!Oa)throw new Error("Missing handflag weights");let Ra=x(La),Fa=x(Oa),Ie=l(Ra.byteLength,y),ze=l(Fa.byteLength,y),Ia=l(12,L);a.queue.writeBuffer(Ie,0,Ra),a.queue.writeBuffer(ze,0,Fa),a.queue.writeBuffer(Ia,0,new Uint32Array([1,288,1]));let za=o.get("handedness.weight"),qa=o.get("handedness.bias");if(!za||!qa)throw new Error("Missing handedness weights");let $a=x(za),Na=x(qa),qe=l($a.byteLength,y),$e=l(Na.byteLength,y),Ya=l(12,L);a.queue.writeBuffer(qe,0,$a),a.queue.writeBuffer($e,0,Na),a.queue.writeBuffer(Ya,0,new Uint32Array([1,288,1]));let Xa=o.get("reg_3d.weight"),Za=o.get("reg_3d.bias");if(!Xa||!Za)throw new Error("Missing reg_3d weights");let Va=x(Xa),Ka=x(Za),Ne=l(Va.byteLength,y),Ye=l(Ka.byteLength,y),ja=l(12,L);a.queue.writeBuffer(Ne,0,Va),a.queue.writeBuffer(Ye,0,Ka),a.queue.writeBuffer(ja,0,new Uint32Array([1,288,63]));let ae=Ct.map(t=>{let{inCh:s,outCh:n,h:i,w:e,stride:c,prefix:d}=t,_=c===2?i/2:i,p=c===2?e/2:e,u=c===2?1:2,r=o.get(`${d}convs.0.weight`),m=o.get(`${d}convs.0.bias`),P=o.get(`${d}convs.1.weight`),C=o.get(`${d}convs.1.bias`);if(!r||!m||!P||!C)throw new Error(`Missing weights for ${d}`);let se=x(r),ue=x(m),oe=x(P),pe=x(C),de=l(se.byteLength,y),ce=l(ue.byteLength,y),J=l(oe.byteLength,y),Y=l(pe.byteLength,y),he=l(32,L),be=l(36,L);return a.queue.writeBuffer(de,0,se),a.queue.writeBuffer(ce,0,ue),a.queue.writeBuffer(J,0,oe),a.queue.writeBuffer(Y,0,pe),a.queue.writeBuffer(he,0,new Uint32Array([1,s,i,e,_,p,c,u])),a.queue.writeBuffer(be,0,new Uint32Array([1,s,n,_,p,Math.max(0,n-s),c,i,e])),{dwWeight:de,dwBias:ce,pwWeight:J,pwBias:Y,dwUniform:he,pwUniform:be,spec:t,outH:_,outW:p}});function ne(t){let s=l(t.length*4,L);return a.queue.writeBuffer(s,0,new Uint32Array(t)),s}let ei=ne([1,96,8,8,16,16]),ai=ne([1,96,16,16,32,32]),ti=ne([1,48,32,32,64,64]);ne([1536*16]),ne([3072*32]),ne([3072*64]);let Ja=S(ra,[fa,fe,ha]),Qa=S(sa,[fe,Sa,Ga,G,Ca]),I=[],z=[],q=[],$=[];for(let t of ae)I.push(S(Te,[G,t.dwWeight,t.dwBias,te,t.dwUniform])),z.push(S(Le,[te,G,t.pwWeight,t.pwBias,O,t.pwUniform])),q.push(S(Te,[O,t.dwWeight,t.dwBias,te,t.dwUniform])),$.push(S(Le,[te,O,t.pwWeight,t.pwBias,G,t.pwUniform]));let ii=S(ve,[G,ya,O,ei]),ni=S(ve,[G,ga,O,ai]),ri=S(ua,[G,Ea,Ha,xa,Ta]),si=S(ve,[xa,ba,O,ti]);S(me,[G,Ie,ze,ie,Ia]),S(me,[G,qe,$e,ie,Ya]),S(me,[G,Ne,Ye,ie,ja]);let K=S(oa,[F.createView(),fe,va]),ui=S(pa,[G,Ie,ze,qe,$e,Ne,Ye,ie]),Xe=24,et=[],at=[];for(let t=Xe;t<ae.length;t++){let s=ae[t];et.push(S(Oe,[G,s.dwWeight,s.dwBias,s.pwWeight,s.pwBias,O,s.dwUniform])),at.push(S(Oe,[O,s.dwWeight,s.dwBias,s.pwWeight,s.pwBias,G,s.dwUniform]))}let Ze=new OffscreenCanvas(9,8).getContext("2d",{willReadFrequently:!0});Ze.globalCompositeOperation="copy";let tt=new OffscreenCanvas(9,8),Ue=tt.getContext("webgpu"),Se=null,Ve=null;if(Ue){Ue.configure({device:a,format:"rgba8unorm",alphaMode:"premultiplied"});let t=a.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"read-only-storage"}}]}),s=a.createShaderModule({code:Ut}),n=a.createShaderModule({code:St});Se=a.createRenderPipeline({layout:a.createPipelineLayout({bindGroupLayouts:[t]}),vertex:{module:s,entryPoint:"vs"},fragment:{module:n,entryPoint:"fs",targets:[{format:"rgba8unorm"}]}}),Ve=a.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:ie}}]})}let Ge=new Float32Array(1),Ce=new Float32Array(1),De=new Float32Array(63);function N(t,s){let n=!0,i=0,e=t.beginComputePass();for(e.setPipeline(ma),e.setBindGroup(0,Qa),e.dispatchWorkgroups(Math.ceil(128/8),Math.ceil(128/8),24);i<=Pi;i++){let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n}e.end();let c=n?G:O;for(t.copyBufferToBuffer(c,0,ba,0,3072*64*4),e=t.beginComputePass();i<=ki;i++){let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n}e.end();let d=n?G:O;for(t.copyBufferToBuffer(d,0,ga,0,3072*32*4),e=t.beginComputePass();i<=Bi;i++){let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n}e.end();let _=n?G:O;for(t.copyBufferToBuffer(_,0,ya,0,1536*16*4),e=t.beginComputePass();i<=Ui;i++){let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n}e.setPipeline(Re),e.setBindGroup(0,ii),e.dispatchWorkgroups(Math.ceil(16/8),Math.ceil(16/8),96),e.end(),n=!1,e=t.beginComputePass();{let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n,i++}e.setPipeline(Re),e.setBindGroup(0,ni),e.dispatchWorkgroups(Math.ceil(32/8),Math.ceil(32/8),96),e.end(),n=!1,e=t.beginComputePass();{let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n,i++}for(e.setPipeline(jt),e.setBindGroup(0,ri),e.dispatchWorkgroups(Math.ceil(32/8),Math.ceil(32/8),48),e.setPipeline(Re),e.setBindGroup(0,si),e.dispatchWorkgroups(Math.ceil(64/8),Math.ceil(64/8),48),e.end(),n=!1,e=t.beginComputePass();i<Xe;i++){let p=n?I[i]:q[i],u=n?z[i]:$[i],r=Z[i];e.setPipeline(r.dwPipeline),e.setBindGroup(0,p),e.dispatchWorkgroups(r.dwDispatchX,r.dwDispatchY,r.dwDispatchZ),e.setPipeline(r.pwPipeline),e.setBindGroup(0,u),e.dispatchWorkgroups(r.pwDispatchX,r.pwDispatchY,r.pwDispatchZ),n=!n}for(;i<ae.length;i++){let p=i-Xe,u=n?et[p]:at[p],r=ae[i];e.setPipeline(Qt),e.setBindGroup(0,u),e.dispatchWorkgroups(r.outW,r.outH,1),n=!n}e.setPipeline(Jt),e.setBindGroup(0,ui),e.dispatchWorkgroups(1),e.end(),s&&t.copyBufferToBuffer(ie,0,s,0,260)}async function Ae(t){a.queue.writeBuffer(fa,0,t);let s=a.createCommandEncoder();{let e=s.beginComputePass();e.setPipeline(wa),e.setBindGroup(0,Ja),e.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),3),e.end()}N(s,B),a.queue.submit([s.finish()]);let n=B.mapAsync(GPUMapMode.READ);await a.queue.onSubmittedWorkDone(),await n;let i=new Float32Array(B.getMappedRange());return Ge[0]=i[0],Ce[0]=i[1],De.set(i.subarray(2,65)),B.unmap(),{handflag:new Float32Array(Ge),handedness:new Float32Array(Ce),landmarks:new Float32Array(De)}}async function Ke(t){a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let s=a.createCommandEncoder();{let e=s.beginComputePass();e.setPipeline(V),e.setBindGroup(0,K),e.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),1),e.end()}N(s,B),a.queue.submit([s.finish()]);let n=B.mapAsync(GPUMapMode.READ);await a.queue.onSubmittedWorkDone(),await n;let i=new Float32Array(B.getMappedRange());return Ge[0]=i[0],Ce[0]=i[1],De.set(i.subarray(2,65)),B.unmap(),{handflag:new Float32Array(Ge),handedness:new Float32Array(Ce),landmarks:new Float32Array(De)}}async function it(t){if(!Se||!Ve||!Ue)throw new Error("Render-based readback not available (no WebGPU canvas context)");a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let s=a.createCommandEncoder();{let p=s.beginComputePass();p.setPipeline(V),p.setBindGroup(0,K),p.dispatchWorkgroups(16,16,1),p.end()}N(s,null);let n=Ue.getCurrentTexture(),i=s.beginRenderPass({colorAttachments:[{view:n.createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:1}}]});i.setPipeline(Se),i.setBindGroup(0,Ve),i.draw(3),i.end(),a.queue.submit([s.finish()]),await a.queue.onSubmittedWorkDone(),Ze.drawImage(tt,0,0);let c=Ze.getImageData(0,0,9,8).data,d=new Float32Array(65),_=new DataView(new ArrayBuffer(4));for(let p=0;p<65;p++){let u=p*4;_.setUint8(0,c[u]),_.setUint8(1,c[u+1]),_.setUint8(2,c[u+2]),_.setUint8(3,c[u+3]),d[p]=_.getFloat32(0)}return{handflag:new Float32Array([d[0]]),handedness:new Float32Array([d[1]]),landmarks:new Float32Array(d.subarray(2,65))}}let oi=a.createBuffer({size:260,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),je=0,pi=[B,oi],re=null,j=null;async function Je(t){let s=pi[je];je=1-je,a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let n=a.createCommandEncoder();{let e=n.beginComputePass();e.setPipeline(V),e.setBindGroup(0,K),e.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),1),e.end()}N(n,s),a.queue.submit([n.finish()]);let i=null;if(re!==null&&j!==null){await re;let e=new Float32Array(j.getMappedRange());i={handflag:new Float32Array([e[0]]),handedness:new Float32Array([e[1]]),landmarks:new Float32Array(e.subarray(2,65))},j.unmap()}return j=s,re=s.mapAsync(GPUMapMode.READ),i}async function nt(){if(!re||!j)return null;await re;let t=new Float32Array(j.getMappedRange()),s={handflag:new Float32Array([t[0]]),handedness:new Float32Array([t[1]]),landmarks:new Float32Array(t.subarray(2,65))};return j.unmap(),re=null,j=null,s}async function di(t=50){let s=new Float32Array(196608);for(let e=0;e<5;e++)await Ae(s);let n=[];for(let e=0;e<t;e++){let c=performance.now();await Ae(s),n.push(performance.now()-c)}let i=n.reduce((e,c)=>e+c,0)/n.length;return{avgMs:i,fps:1e3/i}}async function ci(t=50){let s=new Float32Array(196608);for(let d=0;d<5;d++)await Ae(s);let n=[];for(let d=0;d<t;d++){let _=a.createCommandEncoder();{let u=_.beginComputePass();u.setPipeline(wa),u.setBindGroup(0,Ja),u.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),3),u.end()}N(_,B);let p=performance.now();a.queue.submit([_.finish()]),await a.queue.onSubmittedWorkDone(),n.push(performance.now()-p)}n.sort((d,_)=>d-_);let i=n.reduce((d,_)=>d+_,0)/n.length,e=n[Math.floor(n.length/2)],c=n[0];return{avgMs:i,fps:1e3/i,medianMs:e,minMs:c}}function _i(t){a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let s=a.createCommandEncoder();{let n=s.beginComputePass();n.setPipeline(V),n.setBindGroup(0,K),n.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),1),n.end()}N(s,B),a.queue.submit([s.finish()])}async function li(t,s=50){function n(u){let r=[...u].sort((m,P)=>m-P);return{median:r[Math.floor(r.length/2)],min:r[0]}}for(let u=0;u<10;u++)await Ke(t);let i=[];for(let u=0;u<s;u++){a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let r=a.createCommandEncoder();{let P=r.beginComputePass();P.setPipeline(V),P.setBindGroup(0,K),P.dispatchWorkgroups(16,16,1),P.end()}N(r,B);let m=performance.now();a.queue.submit([r.finish()]),await a.queue.onSubmittedWorkDone(),i.push(performance.now()-m)}let e=[];for(let u=0;u<s;u++){a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let r=a.createCommandEncoder();{let C=r.beginComputePass();C.setPipeline(V),C.setBindGroup(0,K),C.dispatchWorkgroups(16,16,1),C.end()}N(r,B),a.queue.submit([r.finish()]);let m=B.mapAsync(GPUMapMode.READ),P=performance.now();await a.queue.onSubmittedWorkDone(),await m,B.getMappedRange(),B.unmap(),e.push(performance.now()-P)}let c=[];for(let u=0;u<s;u++){a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);let r=a.createCommandEncoder();{let P=r.beginComputePass();P.setPipeline(V),P.setBindGroup(0,K),P.dispatchWorkgroups(16,16,1),P.end()}N(r,B),a.queue.submit([r.finish()]);let m=performance.now();await B.mapAsync(GPUMapMode.READ),B.getMappedRange(),B.unmap(),c.push(performance.now()-m)}let d=[];for(let u=0;u<s;u++){let r=performance.now();await Ke(t),d.push(performance.now()-r)}await Je(t);let _=[];for(let u=0;u<s;u++){let r=performance.now();await Je(t),_.push(performance.now()-r)}await nt();let p=null;if(Se){let u=[];for(let r=0;r<s;r++){let m=performance.now();await it(t),u.push(performance.now()-m)}p=n(u)}return{gpuOnly:n(i),mapAsyncOnly:n(e),mapAsyncNoWait:n(c),total:n(d),pipelined:n(_),renderReadback:p}}async function wi(t){let s=[];async function n(e,c,d){let _=a.createBuffer({size:c,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),p=a.createCommandEncoder();p.copyBufferToBuffer(e,0,_,0,c),a.queue.submit([p.finish()]),await a.queue.onSubmittedWorkDone(),await _.mapAsync(GPUMapMode.READ);let u=new Float32Array(_.getMappedRange()),r=1/0,m=-1/0,P=0;for(let C=0;C<u.length;C++)u[C]<r&&(r=u[C]),u[C]>m&&(m=u[C]),u[C]!==0&&P++;_.unmap(),_.destroy(),s.push({layer:d,stats:{min:r,max:m,nonZero:P,total:u.length}})}a.queue.copyExternalImageToTexture({source:t},{texture:F},[256,256]);{let e=a.createCommandEncoder(),c=e.beginComputePass();c.setPipeline(V),c.setBindGroup(0,K),c.dispatchWorkgroups(Math.ceil(256/16),Math.ceil(256/16),1),c.end(),a.queue.submit([e.finish()])}await n(fe,Math.min(fe.size,3*257*257*4),"canvas\u2192bufInput");{let e=a.createCommandEncoder(),c=e.beginComputePass();c.setPipeline(ma),c.setBindGroup(0,Qa),c.dispatchWorkgroups(Math.ceil(128/8),Math.ceil(128/8),24),c.end(),a.queue.submit([e.finish()])}await n(G,Math.min(G.size,3072*128*4),"inputConv\u2192bufA");let i=!0;for(let e=0;e<Math.min(ae.length,6);e++){let c=i?I[e]:q[e],d=i?z[e]:$[e],_=Z[e],p=ae[e];{let r=a.createCommandEncoder(),m=r.beginComputePass();m.setPipeline(_.dwPipeline),m.setBindGroup(0,c),m.dispatchWorkgroups(_.dwDispatchX,_.dwDispatchY,_.dwDispatchZ),m.end(),a.queue.submit([r.finish()])}await n(te,Math.min(te.size,p.spec.inCh*p.outH*p.outW*4),`layer${e}.DW\u2192bufDW (${p.spec.prefix})`);{let r=a.createCommandEncoder(),m=r.beginComputePass();m.setPipeline(_.pwPipeline),m.setBindGroup(0,d),m.dispatchWorkgroups(_.pwDispatchX,_.pwDispatchY,_.pwDispatchZ),m.end(),a.queue.submit([r.finish()])}let u=i?O:G;await n(u,Math.min(u.size,p.spec.outCh*p.outH*p.outW*4),`layer${e}.PW\u2192buf${i?"B":"A"} (${p.spec.prefix})`),i=!i}return s}return{device:a,run:Ae,runFromCanvas:Ke,runFromCanvasViaRender:it,runFromCanvasPipelined:Je,flushPipelined:nt,benchmark:di,benchmarkGPU:ci,benchmarkDiagnostic:li,debugLayerOutputs:wi,_device:a,_benchmarkSubmitOnly:_i}}var Si="https://cdn.jsdelivr.net/npm/@svenflow/micro-handpose@latest/weights";async function Gi(o={}){let{weightsUrl:w,scoreThreshold:h=.5,forceF32:b=!1}=o;if(typeof navigator>"u"||!navigator.gpu)throw new Error("micro-handpose requires WebGPU. Check browser support at https://webgpureport.org");let U=w??Si,a=U.endsWith("/")?U:`${U}/`,W=`${a}weights_f16.json`,R=`${a}weights_f16.bin`,[E,M]=await Promise.all([fetch(W),fetch(R)]);if(!E.ok)throw new Error(`Failed to fetch weights metadata: ${E.status}`);if(!M.ok)throw new Error(`Failed to fetch weights binary: ${M.status}`);let x=await E.json(),X=await M.arrayBuffer(),g=Dt(x,X),H=await ia(g,{forceF32:b});if(!b){let f=new OffscreenCanvas(256,256),v=f.getContext("2d");v.fillStyle="#886644",v.fillRect(0,0,256,256),v.fillStyle="#cc9966",v.fillRect(50,50,156,156);let k=await H.runFromCanvas(f);k.landmarks.every(Q=>Q===0)&&k.handflag.every(Q=>Q===0)&&(console.warn("[micro-handpose] f16 model produced all-zero output \u2014 recompiling with f32"),H.device.destroy(),H=await ia(g,{forceF32:!0}))}let D=null;function He(){return D||(D=new OffscreenCanvas(256,256)),D}async function y(f){if(f instanceof HTMLCanvasElement||f instanceof OffscreenCanvas||typeof ImageBitmap<"u"&&f instanceof ImageBitmap)return f;let v=He();v.width=256,v.height=256;let k=v.getContext("2d");return f instanceof ImageData?k.putImageData(f,0,0):k.drawImage(f,0,0,256,256),v}function _e(f,v,k){let we=f[0];if(we<h)return null;let Q=v[0]>.5,xe=[];for(let ee=0;ee<21;ee++)xe.push({x:k[ee*3],y:k[ee*3+1],z:k[ee*3+2]});return{score:we,handedness:Q?"right":"left",landmarks:xe}}async function le(f){let v=await y(f),k=await H.runFromCanvas(v);return _e(k.handflag,k.handedness,k.landmarks)}async function ye(f){let v=await y(f),k=await H.runFromCanvasPipelined(v);return k?_e(k.handflag,k.handedness,k.landmarks):null}async function L(){let f=await H.flushPipelined();return f?_e(f.handflag,f.handedness,f.landmarks):null}function l(){H.device.destroy(),D=null}async function S(f){let v=await y(f);return H.benchmarkDiagnostic(v)}async function T(f){let v=await y(f);return H.debugLayerOutputs(v)}return{detect:le,detectPipelined:ye,flushPipelined:L,dispose:l,benchmarkDiagnostic:S,debugLayerOutputs:T}}export{Gi as createHandpose};
